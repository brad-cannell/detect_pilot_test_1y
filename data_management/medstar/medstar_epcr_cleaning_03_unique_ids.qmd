...
-   All 15 entries which failed to receive an automatic group ID were manually reviewed and assigned group IDs

    -   Resulted in 15 new Unique Subject IDs
...
```{r}
# fastLink Output
fl_out <- readRDS(here("data","DETECT Shared GRAs","medstar_cleaning",
                             "medstar_epcr_02_fastlinkobj.rds"))

# Stacked Pairs with Posteriors > 0.54
stacked_pairs <- readRDS(here("data","DETECT Shared GRAs","medstar_cleaning",
                             "medstar_epcr_02_stacked_pairs.rds"))

# Original ID assignments
id_base <- readRDS(here("data","DETECT Shared GRAs","medstar_cleaning",
                             "medstar_epcr_02_id_initial.rds"))
```
...
## Subjects without ID Assignment

There were 15 subjects that somehow evaded the automated group ID assignment.

```{r}
sum(is.na(id_base$id))
```

These 15 observations were flagged for manual review.

```{r}
id_base <- id_base %>%
  mutate(flag_missing = ifelse(is.na(id), TRUE, FALSE)) %>%
  mutate(flag = ifelse(flag_missing, TRUE, flag))

get_unique_value_summary(id_base, "flag_missing")
```

## Duplicated Observations by PCR Report Number `response_pcr`

There were 1,662 duplicated observations, representing 619 PCR report numbers in 293 groups. The 1,675 observations that contained `ms_epcr_row` values or `id` values associated with these duplicates were previously flagged for review.
...
## Posterior Probability in Manual Verification Range of 0.54 - 0.903

The 1586 observations (793 pairs) represented in potential pairs with posterior probabilities in the manual verification range of 0.54 - 0.903 were previously flagged for manual review. This produced an additional 524 observations in this range that were not already flagged for association with a duplicated PCR number.
...
These 2,214 values were isolated to facilitate manual review.

```{r}
checking <- subset(id_base, select= c("id","ms_epcr_row","pt_name_full","pt_dob","pt_gender","pt_race","pt_hispanic","pt_address_num","pt_address_street","pt_address_unit","pt_address_city","flag","flag_duplicate","flag_posterior","flag_missing")) %>%
  filter(flag)
```
...
The 15 observations that failed to receive an automatic subject ID were further isolated and manually reviewed.

```{r}
checking_cols <- filter(checking, flag_missing)
```

These observations were found to be missing nearly all identifying information. This was further confirmed in checking the full data for these observations.

```{r}
checking_cols <- filter(id_base, flag_missing)
```

These observations were set aside to permit checking for potential matches to the limited data during the remainder of the cleaning process.
...
```{r}
checking_cols <- filter(checking, !flag_duplicate & !flag_missing)
...
## Adding Subject IDs for Observations without Subject ID

No feasible matches were identified for the 15 observations that did not receive an automatic ID assignment. These observations were provided individual, separate group IDs.

```{r}

target_rows <- c(18877, 15585, 4907, 33040, 30842, 22809, 16169, 28825, 29083, 
                 29284, 32437, 16660, 24407, 2859, 26399)

ids <- seq(16710,length(target_rows)+16709,by=1)
...
We verified that our map for replacing ID values had the expected 719 observations, with no duplicates or missing (`NA`) values for `ms_epcr_row`.
...
After manually checking non-duplicated and duplicated observations within the verification posterior probability range of 0.54 - 0.903, there were 1156 observations in the posterior probability range flagged for review.

```{r}
get_unique_value_summary(id_base,c("flag","flag_duplicate","flag_posterior"))
```

There were 363 unique IDs present in the data subset at this time. As previous cleaning had focused on ensuring that each unique subject ID in range did not refer to two separate individuals (separating false matches) and deduplication, focus was shifted to assessing for failure to match.
...
# Conversion of ZIP Code to Factor

Values for Patient ZIP Code, `pt_address_zip`, had previously been converted to numeric for fastLink numeric matching. In order to reverse the loss of data and format (due to truncation of leading zeroes), we reconverted the variable to character, reappended leading zeroes (twice, to ensure even the "shortest" zip codes in the US would be caught), then converted to an unordered factor.

```{r}
id_base$pt_address_zip <- as.character(id_base$pt_address_zip)

id_base <- id_base %>%
  mutate(pt_address_zip = case_when(
    is.na(pt_address_zip) ~ pt_address_zip,
...
id_base$pt_address_zip <- factor(id_base$pt_address_zip, ordered = FALSE)
```
...
```{r}
saveRDS(id_base,here("data","DETECT Shared GRAs","medstar_cleaning",

...
-   Both data sets were slightly modified and exported
    ...
    -   The MedStar data set recieved two new variables (total number of Responses and APS Case Numbers associated with the Responses for each subject), and was reordered
...
```{r}
merged_set <- rows_update(merged_set, aps_modified, by=c('id', 'aps_person_id', 'ms_id'))
```
...
detect_cols <- c(ms_detect_vars[!(ms_detect_vars %in% 
                                    c('detect_report_num', 
...
In the MedStar data set, we converted any "Unable to Assess" DETECT item to a missing value, which allowed us to sum the total of "YES" responses for each detect item across all encounters for a subject. We also summed the number of missing DETECT variables to determine the number of screens performed on each subject. We reduced the data set to a single row per Subject, and excluded the unnecessary Row and Case Identifiers, as well as any other variables which were specific to individual Responses.

```{r}
ms_modified <- medstar
ms_modified$na_count <- rowSums(is.na(ms_modified[,detect_questions]))

ms_modified <- ms_modified %>%
  mutate(detect_report_made = case_when(detect_report_made == "YES" ~ "YES",
                                        !is.na(detect_report_num) ~ "YES",
                                        detect_report_made == "NO" ~ "NO",
                                        is.na(detect_report_made) ~ NA_character_)
         ) %>%
  mutate(across(all_of(detect_cols),
         ~ case_when(.x == "YES" ~ TRUE,
                     .x == "NO" ~ FALSE,
                     .x == "UNABLE TO ASSESS" ~ NA)
                )
         ) %>%
  mutate(detect_screen = if_else(na_count < length(detect_questions), TRUE, 
                                 FALSE)
         )
...
ms_modified <- ms_modified %>%
  mutate(ms_subj_num_positive = ifelse(yes_count > 0, TRUE, FALSE)
         ) %>%
  group_by(id) %>%
  mutate(across(c(all_of(detect_cols), ms_subj_num_positive),
                ~ sum(.x, na.rm=TRUE)
                )
         ) %>%
  mutate(ms_subj_screen_count = sum(detect_screen)
  ) %>%
  slice_head(n = 1) %>%
  ungroup() %>%

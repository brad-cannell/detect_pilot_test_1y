...
We cannot perform summary statistics of our List Columns (`comp_report_num_list` and `comp_report_created_list`), so we dropped them prior to codebook processing.

```{r}
ms_data <- ms_data %>%
  subset(select = -c(comp_report_num_list, comp_report_created_list))
```
...
phi_vars <- c(pt_name_vars, pt_addr_vars, pt_dob_vars, pt_rec_ids, 
              report_num_vars, other_phi_vars)
```

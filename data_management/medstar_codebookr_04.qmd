...
Our MedStar data contained 28228 observations of 79 variables. There were 16578 unique subject IDs, and no duplicate PCR Reports.
...
other_phi_vars <- ms_data %>%
  select(detect_report_comment, response_dt, comp_report_created_dt,
         ...
         aps_person_id, aps_case_num, ms_subj_earliest, ms_subj_latest) %>%
  names()
...
epcr_vars <- ms_data %>%
  select(ms_id, ms_epcr_row, pt_name_full, pt_name_first, pt_name_middle, 
         ...
         response_dispo, response_ems_epcr_sig, ms_subj_earliest, ms_subj_latest
         ) %>%
  names() 
...
calc_detect_vars <- ms_data %>%
  select(detect_screened, detect_positive) %>%
  names()

calc_subj_vars <- ms_data %>%
...
for (i in aps_vars){
  attr(ms_data[[i]], "source") <- "Texas Adult Protective Services Data"
}

for (i in calc_detect_vars) {
  attr(ms_data[[i]], "source") <- "Calculated from DETECT Tool Questionnaire (MedStar ePCR Data)"
}

for (i in calc_subj_vars) {
  attr(ms_data[[i]], "source") <- "Calculated from MedStar ePCR Data"
}
```
...
for (i in c('id', 'aps_person_id', 'ms_id')){
  attr(ms_data[[i]], "col_type") <- "Subject ID"
}
```
...
All variables relating to APS data were absent in the data if there was not a matched APS Subject found in the APS data. Skip patterns and descriptions were added.
...
ms_data <- ms_data %>%
  cb_add_col_attributes(
    ...
    skip_pattern = "Only present if a matching Subject was found in the APS data set"
  ) %>%
  cb_add_col_attributes(
    aps_case_num,
    description = "APS Case Number for the APS Intake associated with a specific response",
    skip_pattern = "Only present if a matching APS Intake was found in the APS data set"
  ) %>%
  cb_add_col_attributes(
    aps_row,
    description = "Row in the APS data set that corresponds to the APS Intake that matches a particular response",
    skip_pattern = "Only present if a matching APS Intake was found in the APS data set"
  )
```
...
ms_data <- ms_data %>%
  ...
  cb_add_col_attributes(
    response_date,
    col_type = "Date",
    description = "Date of the EMS Reponse"
  ) %>%
  cb_add_col_attributes(
    response_year,
    col_type = "Year",
...
```{r}
print(book, here("docs", "codebooks", "medstar_codebook_04.docx"))
```

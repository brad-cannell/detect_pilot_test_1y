...
medstar_pairs <- review_matches_wide %>% 
  select(dataset:Weight, ends_with("match"), diff_address_only)

aps_pairs <- review_matches_wide %>% 
  select(dataset1:Weight1, ends_with("match"), diff_address_only)
...
```{r}
pairs_possible_matches <- review_matches
```

```{r}
rm(review_matches, review_matches_wide)
```
...
How many will be dropped because APS investigation date preceeds the DETECT screening date?

```{r}
matches_wide %>% 
  summarise(
    matches_at_start = n(), # 14,423
    screening_after_investigation = sum(medstar_date > aps_date), # 9,662
    diff = matches_at_start - screening_after_investigation # 4,761
  )
```

We started this phase of the data merge with 14,423 potential matches (not unique people). In 9,662 of those potential matches, the APS investigation occurred before the MedStar screening, leaving 4,761 potential matches where the MedStar screening occurred before the APS investigation.
...
matches_wide %>% 
  ...
  summarise(
    matches_remaining = n(), # 4,761
    longer_than_month = sum(time_to_investigation_mnth > 1), # 2,957
    diff = matches_remaining - longer_than_month # 1,804
  )
```

In 2,957 Of the 4,761 remaining potential matches, the APS investigation date is more than 1 month after the initial MedStar screening, leaving 1,804 potential matches.
...
Move these down to where you are actually using them...
...
non_joined <- medstar_aps_merged %>% 
  select(ends_with("_ms"), ends_with("_aps")) %>% 
  names() %>% 
  print()
```

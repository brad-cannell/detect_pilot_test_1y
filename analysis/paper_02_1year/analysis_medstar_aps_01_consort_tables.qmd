...
# Case-Based Temporal Matching

## Overview

There were a total of 28,228 MedStar Responses in our data set. This represented 16,565 Subjects, 1,967 of which were also represented in the APS data set. There were 492 Responses with a paired APS Intake, associated with 461 APS Case Numbers. There were 922 temporally associated APS Case Numbers.

```{r}
nrow(case_data)
length(unique(case_data$id))
...
length(unique(na.omit(case_data$aps_case_num)))
```
...
There were 4,221 Observations without a single DETECT Tool variable entered. This included 2,769 Subjects. There were 28 paired APS Intakes for these observations, and 152 associated APS Case Numbers.
...
There were 24,007 Observations with at least one DETECT Tool variable entered (inclusive of APS Report Number and if a report was made). This included 15,212 Subjects. There were 464 paired APS Intakes for these observations and 881 associated APS Case Numbers.
...
A screening was considered positive if at least one of the DETECT variables (not including that a report was made, or a report number) was selected as "YES". This included 871 observations of 749 Subjects. There were 321 paired APS Intakes for these observations and representing 351 associated APS Case Numbers.
...
The Positive Screenings where it was either indicated a report was made or a report number was recorded in the DETECT tool were further isolated. This included 328 observations of 292 Subjects. There were 273 paired APS Intakes for these observations and 271 associated APS Case Numbers.
...
The 273 reported observations with a paired APS Case Number were further isolated. This subset contained 245 Subjects and 262 associated APS Case Numbers.
...
The 55 remaining reported observations without a paired APS Intake were further isolated. This subset contained 54 Subjects, of which 26 were present in the APS data set. There were 13 associated APS Case Numbers.
...
Within this subset, there were 11 responses associated with an APS Case Number that had at least one valid determination of elder mistreatment across any APS Case Number within 30 days (even if not directly matched to the specific response), 2 that were not, and 42 which lacked a temporally linked APS Case Number. More broadly, there were 25 responses associated with a subject with at least one valid allegation of elder mistreatment across any entry in the APS data set (even if not temporally or directly linked), 2 that were not, and 25 that referenced a subject that was not present in the APS data set. 

```{r}
get_unique_value_summary(checking_subset2, 
                         c('aps_cases_any_valid', 'aps_subject_any_valid'))
```
...
The Positive Screenings where it was indicated that a report was *not* made and no report number was recorded in the DETECT tool were further isolated. This included 542 observations of 493 Subjects. There were 48 paired APS Intakes for these observations, with 97 associated APS Case Numbers.
...
The 493 remaining reported observations without a paired APS Intake were further isolated. This subset contained 458 Subjects, 146 of which were present in the APS data set. There were 146 associated APS Case Numbers.
...
The Positive Screenings where it was neither indicated a report was or was not made (missing value) nor had a report number recorded in the DETECT tool were further isolated. This included 2 observations of 2 Subjects. There were no paired APS Intakes for these observations, but there was a single temporally associated APS Case Number.
...
Within this subset, there was 1 observation that referenced a temporally associated APS Case Number and Subject that had at least one valid determination of elder mistreatment across any APS Case Number within the APS data set (even if not directly matched to the specific response). The remaining observation did not have any determinations made at the subject level, as the subject was missing from the APS data set.

```{r}
get_unique_value_summary(checking_subset, 
                         c('aps_cases_any_valid', 'aps_subject_any_valid'))
```

## Screened Negative

A screening was considered negative if at least of the DETECT variables (not including that a report was made, or a report number) was selected, but no variable was selected as "YES". This included 23,136 observations of 148,881 Subjects. There were 143 paired APS Intakes for these observations and 650 associated APS Case Numbers.
...
The 3 remaining reported observations without a paired APS Intake were further isolated. This subset contained 3 Subjects, none of which were present in the APS data set
...
The Negative Screenings where it was indicated that a report was *not* made and no report number was recorded in the DETECT tool were further isolated. This included 23,106 observations of 14,866 Subjects. There were 136 paired APS Intakes for these observations. There were 1698 Subjects also present in the APS data set, and 643 associated temporally linked APS Case Numbers.
...
nrow(checking_subset)
length(unique(checking_subset$id))
length(unique(na.omit(checking_subset$aps_person_id)))
length(unique(na.omit(checking_subset$aps_row)))
length(unique(na.omit(checking_subset$aps_case_num)))
```
...
The 22,970 remaining reported observations without a paired APS Intake were further isolated. This subset contained 14,831 Subjects, 1,663 of which were also present in the APS data set. There were 572 associated APS Case Numbers.
...
Within this subset, there were 654 responses that had a temporally linked APS Case Number with at least one valid determination of elder mistreatment (even if not directly linked), 299 that did not, and 22,017 that did not have case-level data from the APS data set. More broadly, there were 2,785 observations that referenced a subject that had at least one valid determination of elder mistreatment across any APS Case Number within the APS data set (even if not directly matched to the specific response), 1,123 were not. The remaining 19,062 observations did not have any determinations made at the subject level, as they were missing from the APS data set.
...
The Negative Screenings where it was neither indicated a report was or was not made (missing value) nor had a report number recorded in the DETECT tool were further isolated. This included 20 observations of 20 Subjects. There were no paired APS Intakes or temporally associated APS Case Numbers for these observations.
...
```{r}
get_unique_value_summary(checking_subset, 
                         c('aps_subject_any_valid'))
```

## Not Screened

The 4,221 Responses which were not screened represented 2,769 Subjects, 472 of which were also present in the APS data. There were 152 temporally associated APS Case Numbers.
...
nrow(checking)
length(unique(checking$id))
length(unique(na.omit(checking$aps_person_id)))
length(unique(na.omit(checking$aps_case_num)))
```
...
Of these responses, there were 28 with a paired APS Intake, representing 27 APS Cases across 27 Subjects. There were 4,193 Responses without a paired APS Intake.
...
There were 4,193 responses that lacked a paired APS intake. There were 2,747 Subjects present in the subset, 450 of which were also present in the APS data set. There were 128 temporally associated APS Case Numbers.
...
Within this subset, there were 271 responses that had a temporally linked APS Case Number with at least one valid determination of elder mistreatment (even if not directly linked), 56 that did not, and 3,920 that did not have case-level data from the APS data set. More broadly, there were 662 observations that referenced a subject that had at least one valid determination of elder mistreatment across any APS Case Number within the APS data set (even if not directly matched to the specific response), 231 did not. The remaining 3,300 observations did not have any determinations made at the subject level, as they were missing from the APS data set.
...
Overall, there were 5,490 observations (19.45%) with a subject that was present in the APS data set.
...
nrow(checking)
nrow(checking)*100/nrow(case_data)
```

1,789 (6.34%) observations had a temporally linked APS case number.
...
492 (1.74%) observations had a broadly matched APS Intake.
...
278 (1.16%) observations had an EMS/EMT matched APS Intake.
...
110 (0.46%) of observations were matched to MedStar Compliance data.
...
5,490 (19.49%) of observations were associated with a subject that was present in the APS data set.
...
Of these, 106 (1.93%) were matched with the MedStar Compliance data.
...
nrow(checking_subset)
nrow(checking_subset)*100/nrow(checking)
```

22,738 (80.55%) of observations were associated with a subject that was *not* present in the APS data set.
...
Of these, 4 (0.02%) were matched with the MedStar Compliance data.
...
#### Temporally-Linked APS Case Number

1,789 (6.34%) of observations had a temporally linked APS Case Number.
...
Of these, 105 (5.90%) were matched with the MedStar Compliance data.
...
26,439 (93.66%) of observations *did not have* a temporally linked APS Case Number.
...
Of these, 5 (0.02%) were matched with the MedStar Compliance data.
...
492 (1.74%) of observations had a broadly matched APS Intake.
...
Of these, 101 (20.53%) were matched with the MedStar Compliance data.
...
27,736 (98.26%) of observations *did not have* a broadly matched APS Intake.
...
Of these, 9 (0.03%) were matched with the MedStar Compliance data.
...
278 (0.98%) of observations had an EMS/EMT matched APS Intake.
...
Of these, 93 (33.45%) were matched with the MedStar Compliance data.
...
27,950 (99.02%) of observations *did not have* an EMS/EMT matched APS Intake.
...
Of these, 17 (0.06%) were matched with the MedStar Compliance data.
...
We isolated the 24,007 observations, representing 15,212 subjects, that contained a DETECT screening.
...
4,569 (19.03%) of the screenings were associated with a subject that was present in the APS data set.
...
1,488 (6.198%) of the screenings had a temporally linked APS Case Number.
...
464 (1.93%) of the screenings had a broadly matched APS Intake.
...
271 (1.13%) of the screenings had an EMS/EMT matched APS Intake.
...
105 (0.44%) of the screenings were matched to MedStar Compliance data.
...
4,596 (19.03%) of the screenings were associated with a subject that was present in the APS data set.
...
Of these, 101 (2.21%) were matched with the MedStar Compliance data.
...
19,438 (80.97%) of the screenings were associated with a subject that was *not* present in the APS data set.
...
1,488 (6.20%) of the screenings had a temporally linked APS Case Number.
...
Of these, 100 (6.72%) were matched with the MedStar Compliance data.
...
22,519 (93.80%) of the screenings *did not have* a temporally linked APS Case Number.
...
Of these, 97 (20.91%) were matched with the MedStar Compliance data.
...
23,543 (98.07%) of the screenings *did not have* a broadly matched APS Intake.
...
Of these, 8 (0.03%) were matched with the MedStar Compliance data.
...
Of these, 89 (32.84%) were matched with the MedStar Compliance data.
...
23,736 (98.87%) of the screenings *did not have* an EMS/EMT matched APS Intake.
...
Of these, 16 (0.07%) were matched with the MedStar Compliance data.
...
Of these screenings, 871 (3.63%) were positive ("YES" selected for at least one screening tool item).
...
nrow(positives)
nrow(positives)*100/nrow(screened)
```

104 (11.94%) of the positive screenings were matched to MedStar Compliance data.
...
nrow(checking)
nrow(checking)*100/nrow(positives)
```

#### Subject

517 (59.36%) of the positive screenings were associated with a subject that was present in the APS data set.
...
Of these, 100 (19.34%) were matched with the MedStar Compliance data.
...
354 (40.64%) of the positive screenings were associated with a subject that was *not* present in the APS data set.
...
Of these, 4 (1.13%) were matched with the MedStar Compliance data.
...
392 (45.01%) of the positive screenings had a temporally linked APS Case Number.
...
Of these, 99 (25.26%) were matched with the MedStar Compliance data.
...
479 (54.99%) of the positive screenings *did not have* a temporally linked APS Case Number.
...
Of these, 5 (1.04%) were matched with the MedStar Compliance data.
...
321 (36.85%) of the positive screenings had a broadly matched APS Intake.
...
550 (63.15%) of the positive screenings *did not have* a broadly matched APS Intake.
...
Of these, 7 (1.27%) were matched with the MedStar Compliance data.
...
262 (30.08%) of the positive screenings had an EMS/EMT matched APS Intake.
...
Of these, 89 (33.97%) were matched with the MedStar Compliance data.
...
609 (69.92%) of the positive screenings *did not have* an EMS/EMT matched APS Intake.
...
Of these, 15 (2.46%) were matched with the MedStar Compliance data.
...
Of these screenings, 23,136 (96.37%) were negative ("YES" was not selected for any screening tool item).
...
nrow(negatives)
nrow(negatives)*100/nrow(screened)
```

1 (0.004%) of the negative screenings were matched to MedStar Compliance data.
...
nrow(checking)
nrow(checking)*100/nrow(negatives)
```

#### Subject

4,052 (17.51%) of the negative screenings were associated with a subject that was present in the APS data set.
...
Of these, 1 (0.02%) was matched with the MedStar Compliance data.
...
19,084 (82.49%) of the negative screenings were associated with a subject that was *not* present in the APS data set.
...
Of these, 0 (0%) were matched with the MedStar Compliance data.
...
1096 (4.74%) of the negative screenings had a temporally linked APS Case Number.
...
Of these, 1 (0.09%) was matched with the MedStar Compliance data.
...
22,040 (95.27%) of the negative screenings *did not have* a temporally linked APS Case Number.
...
143 (0.62%) of the negative screenings had a broadly matched APS Intake.
...
22,993 (99.38%) of the negative screenings *did not have* a broadly matched APS Intake.
...
Of these, 1 (0.004%) was matched with the MedStar Compliance data.
...
9 (0.04%) of the negative screenings had an EMS/EMT matched APS Intake.
...
23,127 (99.96%) of the negative screenings *did not have* an EMS/EMT matched APS Intake.
...
We isolated the 4,221 observations, representing 2,769 subjects, that did not contain a DETECT screening.

```{r}
not_screened <- case_data %>%
  filter(!detect_screened)

nrow(not_screened)
length(unique(not_screened$id))
```

### Overall

921 (21.82%) of the non-screening observations were associated with a subject that was present in the APS data set.

```{r}
checking <- not_screened %>%
  filter(!is.na(aps_person_id))

nrow(checking)
nrow(checking)*100/nrow(not_screened)
```

301 (7.13%) of the non-screening observations had a temporally linked APS Case Number.
...
28 (0.66%) of the screenings had a broadly matched APS Intake.
...
7 (0.17%) of the non-screening observations had an EMS/EMT matched APS Intake.
...
5 (0.12%) of the non-screening observations were matched to MedStar Compliance data.
...
Of these, 5 (0.54%) were matched with the MedStar Compliance data.
...
3,300 (78.18%) of the non-screening observations were associated with a subject that was *not* present in the APS data set.
...
Of these, 5 (1.66%) were matched with the MedStar Compliance data.
...
3,920 (92.87%) of the non-screening observations *did not have* a temporally linked APS Case Number.
...
28 (0.66%) of the non-screening observations had a broadly matched APS Intake.
...
Of these, 4 (14.29%) were matched with the MedStar Compliance data.
...
4,193 (99.34%) of the non-screening observations *did not have* a broadly matched APS Intake.
...
Of these, 4 (57.14%) were matched with the MedStar Compliance data.
...
4,214 (99.83%) of the non-screening observations *did not have* an EMS/EMT matched APS Intake.
...
Of these, 1 (0.024%) was matched with the MedStar Compliance data.

---
title: "Cleaning MedStar ePCR - PHI"
html:
...
-   Addition of Response Year, Month, Day, and Time variables from original aggregate variable for Response Date/Time, without removal of original aggregate variable

    -   `response_dt`, `response_year`, `response_month`, `response_day`, `response_time`
...
```{r}
phi_cleaning <- read_csv(here("data","DETECT Shared GRAs","cleaning_with_phi",
                              "medstar","medstar_epcr_phi_cleaning.csv"), show_col_types = FALSE)
```
...
ms_epcr <- ms_epcr %>%
  mutate(across(uppercasing, str_to_upper)
         )
```
...
### Patient Race `pt_race` and `pt_ethnicity`
...
We divided `response_dt` into `response_year`, `response_month`, `response_day`, and `response_time` utilizing the *lubridate* package.
...
We ensured all values for `pt_address_zip`, `pt_dob_year`, `pt_dob_month`, `pt_dob_day`, `response_year`, `response_month`, `response_day`, and `pt_age` are numeric.

```{r}
numerics <- c("pt_address_zip", "pt_dob_year", "pt_dob_month", "pt_dob_day",
              "response_year", "response_day", "response_month", "response_day",
              "pt_age")
...
We drop our flag variables `flag_address` and `flag_name`, as well as temporary variables `pt_name_chunk` and `num_temp` as they are no longer needed.
...
We order our columns, for clarity.
...
resp_detail_cols <- c("response_pcr","response_num","response_dt",
                      ...
                      "response_time","response_complaint","response_symptoms",
...
Only two observations are different in these variables. On manual inspection, the "less complete" version of this call has a disposition of "Canceled False Call", which was likely created in error. This observation also differs in its DETECT data, so it would be caught in selection based on DETECT data completeness.
...
```{r}
ms_epcr$na_count <- rowSums (is.na(ms_epcr[append(detect_binaries,"detect_report_num")]))
...
```{r}
saveRDS(ms_epcr,here("data","DETECT Shared GRAs","cleaning_with_phi",

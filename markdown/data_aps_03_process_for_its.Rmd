...
```{r}
aps <- feather::read_feather("/Volumes/sph_research/Detect/one_year_data/aps.feather")
```
...
```{r}
about_data(aps_its) # 18,080 observations and 49 variables
```
...
```{r}
aps_its <- aps_its %>% 
  select(study_week, county, reporter, ems, detect)
```

```{r}
about_data(aps_its) # 18,080 observations and 5 variables
```

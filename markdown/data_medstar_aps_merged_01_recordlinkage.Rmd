...
```{r}
# For Brad while testing code
medstar_complete <- read_rds("/Users/bradcannell/Desktop/medstar_epcr_02_variable_management.rds")
```
...
```{r}
# For Brad while testing code
aps <- read_rds("/Users/bradcannell/Desktop/aps_02_variable_management.rds")
```
...
medstar_complete <- medstar_complete %>% 
  ...
  group_by(match_vars) %>% 
  mutate(match_var_row = row_number()) %>% 
  ungroup()
```
...
```{r message=FALSE}
medstar_unique <- medstar_complete %>% 
  filter(match_var_row == 1)
```

```{r}
dim(medstar_unique) # 19,186    58
```
...
Imported APS dataset (aps) contains only one row per case_num
...
```{r}
medstar_compare <- medstar_unique %>%
  # The variables you want to compare have to appear in the same order in both datasets
  select(incident_pcr, arrival_time, name_first, name_last, dob, address_num, 
         address_street_name)

aps_compare <- aps_unique %>%
  # The variables you want to compare have to appear in the same order in both datasets
  select(case_num, intake_date, name_first, name_last, dob, address_num, 
         address_street_name)
```

```{r}
ncol(medstar_compare) == ncol(aps_compare) # TRUE
```
...
#   strcmp   = c("name_first", "name_last", "dob", "address_num", "address_street_name"),
...
```{r eval=FALSE}
# create smaller sample size - delete 
set.seed(123)
medstar_compare <- sample_n(medstar_compare, 1000)
aps_compare <- sample_n(aps_compare, 1000)
...
# save test file
saveRDS(pairs_possible_matches, "/Users/chiufengyap/OneDrive - The University of Texas Health Science Center at Houston/Research/MedStar_APS/delete/test_pairs_possible_matches.rds")
```

...
aps <- readxl::read_excel(
  path = "/Volumes/sph_research/Detect/phase_two/aps_data.xlsx"
)
```
...
aps %>% 
  ...
  count() %>% 
  ungroup() %>% 
  select(`Duplicate Rows` = n) # 72 groups of duplicates
```
...
```{r}
feather::write_feather(aps, "/Volumes/sph_research/Detect/phase_two/aps.feather")
```

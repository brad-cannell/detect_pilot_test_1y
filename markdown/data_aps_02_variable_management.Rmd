...
```{r}
dim(aps_unique) # 15,283 observations and 66 variables
```
...
```{r}
dim(aps_unique) # 15,283 observations and 69 variables
```
...
So, there are 2 rows (1 reporet to APS) where a report was made to APS (intake stage number) by two people at the same time, but both individuals were the same type of reporter (both health care providers/staff). Catherine told me that it was possible for two people to call-in at the same time.

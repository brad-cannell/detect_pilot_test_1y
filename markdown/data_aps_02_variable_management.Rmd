...
aps_2 <- aps_2 %>%
  mutate(
    birth_year = format(dob, "%Y"),
    birth_mnth = format(dob, "%m"),
    birth_day  = format(dob, "%d")) %>% 
  group_by(case_num) %>%
  mutate(
    birth_year = Mode(birth_year),
    birth_mnth = Mode(birth_mnth),
    birth_day  = Mode(birth_day)) %>%
  mutate(dob = paste(c(birth_year), c(birth_mnth), c(birth_day), sep="-"),
         dob = as.Date(dob, format="%Y-%m-%d")) %>% 
  select(case_num, case_row, name_last, name_first, name_full, dob, birth_year, birth_mnth, birth_day, everything())
```
...
```{r}
rm(dnf, dnl, dz, list=ls(pattern="^discrepancies_"))
```
...
```{r}
dim(aps_unique) # 15,283 observations and 74 variables
```
...
```{r}
dim(aps_unique) # 15,283 observations and 81 variables
```

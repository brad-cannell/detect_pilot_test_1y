...
Let's use the data with one row per case_num.
...
```{r}
# Change all dob's year LIKE "20__" to "19__" because records with similar case_num that have all dob_all_match = TRUE could have all their dobs LIKE 2027-01-01, which is impossible.
aps <- aps %>% 
  mutate(
...
aps <- subset(aps, select = -c(dob_year, dob_month, dob_day, dob_year_1, dob_year_2))
```
...
```{r}
# View(discrepancies_name_last) # 52 observations
...
```{r}
# View(discrepancies_name_first) #118 observations
...
```{r}
# View(discrepancies_name_full) #139 observations
aps_2 <- aps_2 %>%
...
aps_test <- aps_2 %>%
  group_by(case_num) %>%
  mutate(test_street = all_match(street),
...
# # check for all_match == FALSE
# aps_2 %>% group_by(case_num) %>% mutate(age_test = all_match(age)) %>% filter(age_test==FALSE) # 0 obs
```
...
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    invalid_emotional_abuse_total = sum(invalid_emotional_abuse),
    invalid_emotional_abuse = max(invalid_emotional_abuse))
```

### Discrepancies in invalid_exploitation     

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    invalid_exploitation_total = sum(invalid_exploitation),
    invalid_exploitation = max(invalid_exploitation)) 
```

### Discrepancies in invalid_medical_neglect 

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    invalid_medical_neglect_total = sum(invalid_medical_neglect),
    invalid_medical_neglect = max(invalid_medical_neglect))
```

### Discrepancies in invalid_mental_health_neglect

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    invalid_mental_health_neglect_total = sum(invalid_mental_health_neglect),
    invalid_mental_health_neglect = max(invalid_mental_health_neglect))
```

### Discrepancies in invalid_physical_abuse   

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    invalid_physical_abuse_total = sum(invalid_physical_abuse),
    invalid_physical_abuse = max(invalid_physical_abuse))
```

### Discrepancies in invalid_physical_neglect 

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    invalid_physical_neglect_total = sum(invalid_physical_neglect),
    invalid_physical_neglect = max(invalid_physical_neglect))
```

### Discrepancies in invalid_sexual_abuse   

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    invalid_sexual_abuse_total = sum(invalid_sexual_abuse),
    invalid_sexual_abuse = max(invalid_sexual_abuse))
```

### Discrepancies in other_emotional_abuse  

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    other_emotional_abuse_total = sum(other_emotional_abuse),
    other_emotional_abuse = max(other_emotional_abuse))
```

### Discrepancies in other_exploitation     

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    other_exploitation_total = sum(other_exploitation),
    other_exploitation = max(other_exploitation))
```

### Discrepancies in other_medical_neglect      

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    other_medical_neglect_total = sum(other_medical_neglect),
    other_medical_neglect = max(other_medical_neglect))
```

### Discrepancies in other_mental_health_neglect 

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    other_mental_health_neglect_total = sum(other_mental_health_neglect),
    other_mental_health_neglect = max(other_mental_health_neglect))
```

### Discrepancies in other_physical_abuse 

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    other_physical_abuse_total = sum(other_physical_abuse),
    other_physical_abuse = max(other_physical_abuse))
```

### Discrepancies in other_physical_neglect  

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    other_physical_neglect_total = sum(other_physical_neglect),
    other_physical_neglect = max(other_physical_neglect))
```

### Discrepancies in other_sexual_abuse 

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    other_sexual_abuse_total = sum(other_sexual_abuse),
    other_sexual_abuse = max(other_sexual_abuse))
```

### Discrepancies in total_allegations_investigated
...
### Discrepancies in total_other_dispositions
...
### Discrepancies in utd_emotional_abuse

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    utd_emotional_abuse_total = sum(utd_emotional_abuse),
    utd_emotional_abuse = max(utd_emotional_abuse))
```

### Discrepancies in utd_exploitation

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    utd_exploitation_total = sum(utd_exploitation),
    utd_exploitation = max(utd_exploitation))
```

### Discrepancies in utd_medical_neglect

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    utd_medical_neglect_total = sum(utd_medical_neglect),
    utd_medical_neglect = max(utd_medical_neglect))
```

### Discrepancies in utd_mental_health_neglect

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    utd_mental_health_neglect_total = sum(utd_mental_health_neglect),
    utd_mental_health_neglect = max(utd_mental_health_neglect)) 
```

### Discrepancies in utd_physical_abuse

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    utd_physical_abuse_total = sum(utd_physical_abuse),
    utd_physical_abuse = max(utd_physical_abuse))
```

### Discrepancies in utd_physical_neglect   

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    utd_physical_neglect_total = sum(utd_physical_neglect),
    utd_physical_neglect = max(utd_physical_neglect))
```

### Discrepancies in utd_sexual_abuse       

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    utd_sexual_abuse_total = sum(utd_sexual_abuse),
    utd_sexual_abuse = max(utd_sexual_abuse)) 
```

### Discrepancies in valid_emotional_abuse   

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    valid_emotional_abuse_total = sum(valid_emotional_abuse),
    valid_emotional_abuse = max(valid_emotional_abuse)) 
```

### Discrepancies in valid_exploitation 

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    valid_exploitation_total = sum(valid_exploitation),
    valid_exploitation = max(valid_exploitation))
```

### Discrepancies in valid_medical_neglect  

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    valid_medical_neglect_total = sum(valid_medical_neglect),
    valid_medical_neglect = max(valid_medical_neglect)) 
```

### Discrepancies in valid_mental_health_neglect

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    valid_mental_health_neglect_total = sum(valid_mental_health_neglect),
    valid_mental_health_neglect = max(valid_mental_health_neglect))
```

### Discrepancies in valid_physical_abuse 

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    valid_physical_abuse_total = sum(valid_physical_abuse),
    valid_physical_abuse = max(valid_physical_abuse))
```

### Discrepancies in valid_physical_neglect 

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    valid_physical_neglect_total = sum(valid_physical_neglect),
    valid_physical_neglect = max(valid_physical_neglect))
```

### Discrepancies in valid_sexual_abuse

```{r}
aps_2 <- aps_2 %>% 
  group_by(case_num) %>% 
  mutate(
    valid_sexual_abuse_total = sum(valid_sexual_abuse),
...
```{r}
# Not all zips include the specific delivery route 4 digits - reformat all zips to only include first 5 digits 
aps_2$zip <- aps_2$zip %>% substr(1,5)

#creating new data frame for ease of writing code in next step
dz <- aps_2

# fill NAs with mode zip by case_num. 
# If there are only two cases, the latest instance's zip will be used.
aps_2 <- aps_2 %>% group_by(case_num) %>% 
  mutate(zip = Mode(zip)) %>% select(case_num,starts_with("zip"), everything()) %>% 
  mutate( 
    ## there are case_num that has NA, manually filling in with available/latest instance's zip
    zip = case_when(
      ...
      case_num == 45431722 ~ dz$zip[dz$case_num == 45431722 & dz$case_row == 3], # latest zip
      TRUE ~ zip
  ))
```
...
# From Brad (Delete when done):

1. Move code to create reporter dummy variables up here.

2. Move the code to create earliest and latest report date.
...
```{r eval=FALSE}
aps_2 %>% 
  freqtables::freq_table(city) # 254 different cities
```
...
```{r}
dim(aps_2) # 18,080 observations and 79 variables
```
...
```{r}
dim(aps_2) # 18,080 observations and 82 variables
```
...
# Earliest : "2014-12-31 10:26:00 UTC"
# Latest:  "2018-02-28 18:09:00 UTC"
...
```{r}
# Data checks
aps_2 %>% select(intake_date, intake_year, year_add, week, study_week, detect)
```
...
```{r}
dim(aps_2) # 18,080 observations and 84 variables
```
...
```{r}
unique(aps_2$reporter) # 14 different types
```
...
aps_2 <- aps_2 %>% 
  ...
  mutate(
    ...
    reporter_community_agencies_organizations        = max(reporter_blank_unknown),
...
dim(aps_2) # 18,080 observations and 98 variables
...
aps_2 <- aps_2 %>% 
  ...
  mutate(reporter_total = sum(reporter_anonymous + reporter_blank_unknown + 
                                ...
                                reporter_other_providers + reporter_others + reporter_other_providers + 
                                reporter_others + reporter_state_agencies + reporter_victim))
```

```{r}
dim(aps_2) # 18,080   101
```
...
aps_2 <- aps_2 %>%
  group_by(case_num) %>%
  # select(case_num, starts_with("valid"), starts_with("invalid")) %>% 
  rowwise() %>% 
  mutate(
...
```{r}
dim(aps_2) # 18,080 observations and 104 variables
```

```{r}
# Data checks
# aps_2 %>% filter(case_num == 44885551)
```
